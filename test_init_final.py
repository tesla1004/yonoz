...
class mainCog(commands.Cog): 
	...
	async def join_(self, ctx):
		...
		if basicSetting[7] == "":
			channel = ctx.message.guild.id #메세지가 들어온 채널 ID
			...
			await ctx.send(f"< 메인서버 [{ctx.message.guild.name}] 접속완료 >\n< 음성채널 접속 후 [{command[5][0]}] 명령을 사용 하세요 >", tts=False)
			
			print('< 메인서버 [' + ctx.guild.get_channel(basicSetting[7]).name + '] 접속완료>')
			...
			for guild in self.bot.guilds:
				if basicSetting[7] == guild.id:
					curr_guild_info = guild

			emoji_list : list = ["⭕", "❌"]
			guild_error_message = await ctx.send(f"이미 **[{curr_guild_info.name}]** 서버가 메인 서버로 설정되어 있습니다.\n해당 서버로 메인 서버를 변경 하시려면 ⭕ 그대로 사용하시려면 ❌ 를 눌러주세요.\n(10초이내 미입력시 기존 설정 그대로 설정됩니다.)", tts=False)
			...
			try:
				...
				return await ctx.send(f"시간이 초과됐습니다. **[{curr_guild_info.name}]** 서버에서 사용해주세요!")

			if str(reaction) == "⭕":
				if ctx.voice_client is not None:
					await ctx.voice_client.disconnect(force=True)
				basicSetting[6] = ""
				basicSetting[7] = int(ctx.message.guild.id)
				...
				return await ctx.send(f"메인서버가 **[{ctx.author.guild.name}]** 서버로 새로 설정되었습니다.\n< 음성채널 접속 후 [{command[5][0]}] 명령을 사용 하세요 >")
			else:
				return await ctx.send(f"메인서버 설정이 취소되었습니다.\n**[{curr_guild_info.name}]** 서버에서 사용해주세요!")
